...
// Creates an activity log entry and broadcasts it to subscribers.
...
export const resolvers = {
  ...
  Mutation: {
    ...
    moveColumn: async (
      ...
      await Promise.all(
        cols.map((c: { _id: unknown }, i: number) =>
          Column.updateOne({ _id: c._id }, { $set: { order: i } })
        )
      );
  ...
  Subscription: {
    ...
    activityUpdated: {
      subscribe: async (
        ...
        await requireBoardAccess(ctx, boardId);
        return pubsub.asyncIterator(activityChannel(boardId));
      },
